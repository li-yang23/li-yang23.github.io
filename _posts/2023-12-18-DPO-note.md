...
layout: post
title: DPO背景知识
description: notes for "Direct Perference Optimization：Your Lnaguage Model is Secretly a Reward Model"
...
$$
\mathcal{L}_{DPO}(\pi_{\theta};\pi_{ref})=-\mathbb{E}_{(x,y_w,y_l)\sim D}\left[\log\sigma(\beta\log\frac{\pi_{\theta}(y_w\vert x)}{\pi_{ref}(y_w\Vert x)}-\beta\log\frac{\pi_{\theta}(y|l\vert x)}{\pi_{ref}(y\vert x)})\right]
$$
