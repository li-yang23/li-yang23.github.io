...
图取消学习是机器取消学习的扩展，专门针对图结构数据而设计。 它涉及完全删除已删除数据的所有痕迹，例如图中的节点和边，包括其所有连接。 换句话说，图遗忘的目标是从图中删除特定的节点和边以及其他设置，同时保留其整体结构。 这种级别的数据删除比传统的机器取消学习要复杂得多，因为它需要解开互连的实体。

图结构数据的复杂性对图的遗忘提出了挑战。 图中节点和边之间的关系错综复杂，形成了一个密集的互连信息网络。为了揭示遗忘数据的影响，同时保留图的结构，需要复杂的算法。这些算法必须能够在不影响图功能的情况下理清这些复杂的连接。就数据隐私和道德机器学习实践而言，随着数据隐私法规和个人控制其数据的权利变得越来越重要，从图结构数据中全面删除敏感信息的能力成为至关重要的道德保障。确保有效、彻底地消除图表中被遗忘的数据可以让个人更好地控制自己的信息，增强对机器学习系统的信心，并维护负责任的数据管理的道德标准。
...
1. Certified Data Removal from Machine Learning Models
   ...
   3. 为使用可微凸损失函数(例如逻辑回归器)训练的L2正则化线性模型开发一种可验证遗忘机制，通过计算遗忘样本点处的Hessian值和损失梯度，使用牛顿更新遗忘机制进行一次牛顿更新完成模型的遗忘过程。
...
2. GraphEditor: An Efficient Graph Representation Learning and Unlearning Approach
...
3. Fast Yet Effective Graph Unlearning through Influence Analysis
...
4. Efficient Model Updates For Approximate Unlearning of Graph-Structured Data
...
2. In-Context Unlearning: Language Models As Few Shot Unlearners
...
1. Graph Unlearning Using Knowledge Distillation
...
1. Certified Graph Unlearning
   1. 也是对SGC这样的线性GNN进行的分析，目前还没看出和GraphEditor的区别
   2. 但也是一个继续训练方法，根据删除的数据计算梯度更新方向，然后直接更新梯度

### USENIX Security

#### 2023

1. Inductive Graph Unlearning
...
2. GIF: A General Graph Unlearning Strategy via Influence Function
...
3. Heterogeneous Federated Knowledge Graph Embedding Learning and Unlearning
...
1. Efficiently Forgetting What You Have Learned in Graph Representation Learning via Projection
...
1. SAFE: Machine Unlearning With Shard Graphs
