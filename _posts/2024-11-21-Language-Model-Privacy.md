...
机器学习模型会从训练数据中提取可泛化的模式，有工作发现记住训练数据中的某些部分是在长尾分布上进行最优泛化的必要条件。数据记忆会直接导致模型训练集中的私人信息泄露，因为模型对训练集中存在的样本的行为变得与不存在的样本明显不同。典型的数据泄露的风险包括：
...
> 也就是上面说的，一个信息是否是隐私信息与其所处的上下文环境，描述的对象和被访问的对象相关。在语言模型中，隐私的泄露可能根本不需要本人的知情，不需要主体进行泄露。比如公司员工登记家人的身份证号，比如共享的八卦信息，都是在本人不知情的情况下出现的泄露。
...
DP隐私分词（tokenization）是另一种隐私保护训练方法，这包括将噪声注入语料库的单词直方图，DP 训练的一些可用性下降可以通过使用DP分词器来恢复。
...
使用 DP 进行训练时，在微调训练语料库上训练的模型性能下降更多。因为微调数据一般规模更小且包含更多敏感信息，在这些数据上平衡可用性和隐私性会更加困难。此外，一个问题是在 DPSGD 期间，噪声被均匀地添加到所有数据点中，这意味着对于数据中的稀有组，收敛将更加困难。如果数据并不均衡，均匀地添加DP噪声可能导致在稀有数据上出现更加明显的表现下滑（是因为噪声强度应当与模型见过数据的次数相关吗？）。解决偏倚的技术（例如反事实数据增强）可以与 DP 相结合，以帮助减轻这种影响。
#### 隐私保护推理
