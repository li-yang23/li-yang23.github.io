...
\begin{equation}
\label{eq:reward-prob}
Q_{r_{\phi}}(y\prec y'\vert x)=\frac{\exp(r_{\phi}(x,y))}{\exp(r_{\phi}(x,y))+\exp(r_{\phi}(x,y'))}
\end{equation}

可以看到\eqref{eq:ranking-loss}的$$\sigma(\cdot)$$里面的东西就是$$Q_{r_{\phi}}(y\prec y'\vert x)$$，所以对于ranking loss的优化可以解释为对于$$Q_{r_{\phi}}$$的最大对数似然：$$\mathcal{L}_{Ranking}(r_{\phi;\mathcal{D}_P})=-\mathbb{E}_{\mathcal{D}_P}\left[\log Q_{r_{\phi}}(y^{good}\prec y^{bad}\vert x)\right]$$
...
KL\left[P(y\prec y'\vert x)\vert\vertQ_{r_{\phi}}(y\prec y'\vert x)\right]<\eta_2
\end{equation}
